import {
  ...
  SET_MODERATOR,
  SET_START_WHITEBOARD,
  SET_STOP_WHITEBOARD,
  SET_START_SHARED_DOCUMENTS,
  SET_STOP_SHARED_DOCUMENTS,
  TOGGLE_PIP,
} from "../actions/types";
  ...
  switch (action.type) {
    case SET_USER_RESOLUTION:
      if (action.payload.resolution) {
        state.resolution[action.payload.participantId] =
    ...
    case SET_MODERATOR:
      if (action.payload.isModerator) {
        state.moderator[action.payload.participantId] =
          action.payload.participantId;
      }
      return { ...state };
    case SET_PRESENTER:
      if (action.payload.presenter) {
        ...
        state.presenterParticipantIds = state.presenterParticipantIds.filter(
          (item) => item !== action.payload.participantId
        );
      }
      return { ...state };
    case SET_PIN_PARTICIPANT:
      state.pinnedParticipant = action.payload.participantId
        ? {
            isPresenter: action.payload.type,
